...
def PLS_correlated_noise(f, fref, pol, snr, Tobs, psi,):
    ...
    # Detector functions
    def N_aet(f, f_pivot_noise, r, n_noise):
        '''
        Noise in the diagonal basis, general for triangular-shaped detectors 
        '''
        f_auto, N_auto_f = detectors.detector_Pn('ET X')
        N_auto_f = np.interp(f, f_auto, N_auto_f)
        N_pivot = interp1d(f, N_auto_f)(f_pivot_noise) 
    ...
    psd_A = N_aet(f, f_pivot_noise, r, n_noise)[0]
    psd_E = N_aet(f, f_pivot_noise, r, n_noise)[1]
    psd_T = N_aet(f, f_pivot_noise, r, n_noise)[2]
