...
def auto(detector, psi, pol): 
    c1, u1, v1,  l1, det_name = det.detector(detector, shift_angle=None)
    gamma = 0
    if (pol == 't'):
        integrand_re = lambda x, y: (1/(4*pi))*(F(x, y, psi, u1, v1)[0]**2 \
        + F(x, y, psi, u1, v1)[1]**2) * sin(x)
    elif (pol == 'v'):
        integrand_re = lambda x, y: (1/(4*pi))*(F(x, y, psi, u1, v1)[2]**2 \
        + F(x, y, psi, u1, v1)[3]**2) * sin(x)
    elif (pol == 's'): #fattore 6 da aggiungere todo*******
        k = 0 # default value (degeneracy among the two scalar pol modes)
        integrand_re = lambda x, y: (3/(2*pi))*(F(x, y, psi, u1, v1)[4]**2)*sin(x)
    # aggiungere una riga di codice per gli errori 
    x_values = np.linspace(0, pi, 100)
    y_values = np.linspace(0, 2*pi, 100)
    X, Y = np.meshgrid(x_values,y_values)
    f_values = integrand_re(X, Y)
    gamma_x = np.trapz(f_values, x_values, axis=0)
...
def overlap_f(detector1, detector2, f, psi, pol, shift_angle=False):
    ...
    with Pool() as pool:
        orf = []
        for result in tqdm(pool.imap(overlap_worker_f, args_list), total=len(args_list), miniters=1):
            orf.append(result)
    if(pol == 't'):
        return orf/norm_t
    elif(pol == 's'):
...
def overlap_all_f(detector1, detector2, f, psi, norm=False, shift_angle=False):
    if(detector1 =='ET L2' and detector2 !='ET L2'):
        ec1, e11, e21, l1, det1_name = det.detector(detector1, shift_angle)
        ec2, e12, e22, l2, det2_name = det.detector(detector2, None)
...
def orf(detector1, detector2, f, psi, norm=False, shift_angle=False, transfer_function=False):
    if transfer_function == False:
        ...
        return overlap_all_f(detector1, detector2, f, psi, norm, shift_angle)
...
def orf_p(detector1, detector2, f, psi, pol, norm=False, shift_angle=False, transfer_function=False):
    if transfer_function == False:
        ...
        return overlap_f(detector1, detector2, f, psi, pol, norm, shift_angle)
