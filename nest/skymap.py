...
def transfer_function(L, e, f, theta, phi, psi):
    ...
    l = e
    exp3 = np.exp(-3j*pi*f*L/c*(1-np.einsum('i,i->', omega, l)))
...
def F_transfer(theta, phi, psi, f, e1, e2, L): #angular pattern function
...
def skymap_apf(psi, det_name1, nside, f, shift_angle=None):
        ...
        for i in tqdm(range(len(theta)), desc="Processing {0} ".format(which_det1)):
            ...
            F1 = F_transfer(theta[i], phi[i], psi, f, u1, v1, l1)
...
def skymap_overlap(psi, det_name1, det_name2, nside, f, shift_angle=None):
        ...
        for i in range(len(theta)):
            F1 = F_transfer(theta[i], phi[i], psi, f, u1, v1, l1)
            F2 = F_transfer(theta[i], phi[i], psi, f, u2, v2, l2)
            cos_term =  np.cos(2*pi*f*(d[0]*Omega(theta[i], phi[i])[0] + d[1]*Omega(theta[i], phi[i])[1] + d[2]*Omega(theta[i], phi[i])[2])/c)
            overlap_t[i] = 5/(8*np.pi)*(F1[0]*F2[0] + F1[1]*F2[1]) #* cos_term 
            overlap_v[i] = 5/(8*np.pi)*(F1[2]*F2[2] + F1[3]*F2[3]) #* np.cos(2*pi*f*(d[0]*Omega(theta[i], phi[i])[0] + d[1]*Omega(theta[i], phi[i])[1] + d[2]*Omega(theta[i], phi[i])[2])/c)
            overlap_s[i] = 3/(2*np.pi)*(F1[4]*F2[4])#* np.cos(2*pi*f*(d[0]*Omega(theta[i], phi[i])[0] + d[1]*Omega(theta[i], phi[i])[1] + d[2]*Omega(theta[i], phi[i])[2])/c)
